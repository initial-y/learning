...
线程需要不断地取尝试获取锁，这样会浪费服务器资源。

## 2. 等待/通知机制
> 等待/通知机制是基于Object的wait()和notify()/notifyAll()方法来实现的。

wait()方法使当前线程进入WATITING状态，此时会释放锁（join不会释放锁）。注意，调用notify()/notifyAll()方法并不会使唤醒的线程释放锁，需要线程自行释放锁或者调用wait()方法释放锁。

> notify()方法会**随机**叫醒一个正在等待的线程，而notifyAll()会叫醒所有正在等待的线程。

需要注意的是，等待/通知机制使用的是同一个对象锁，如果两个线程用的不是同一个对象锁，那它们之间是不能用等待/通知机制的。



## 3. 信号量
> JDK提供的信号量功能的类：Semaphore。
>
> 在多个线程（多于2个）需要相互合作的情况下，用简单的锁和等待/通知机制不那么方便，此时就可以选择使用信号量。

### volatile关键字

volatile关键字保证内存的可见性。用volatile关键字声明的变量，线程之间对变量的值的操作是相互可见的（堆内存共享）。

### 信号量应用场景

多个线程（超过2个）需要相互合作，用简单的“锁”和“等待通知机制”就不那么方便时，这个时候就可以用到信号量。



## 4. 管道
> 管道是基于管道流的通信方式。JDK提供了字符流：PipedWriter、PipedReader，字节流：PipedOutputStream、PipedInputStream。
> 
> 管道的应用场景多半与I/O流相关。比如一个线程向另一个线程发送字符串或者文件等场景就可以使用管道。

### 管道通信应用场景

使用管道多半与I/O流相关。当我们一个线程需要先另一个线程发送一个信息（比如字符串）或者文件等等时，就需要使用管道通信。


# 参考

- [不可不说的Java“锁”事](https://tech.meituan.com/Java_Lock.html)
- [Java线程间的通信](http://concurrent.redspider.group/article/01/5.html)